...
def _is_in_container() -> bool:
    # 逻辑说明：检查 Docker/Podman 标记文件，判断 URL 是否需要按容器网络规则重映射。
...
def _port_remap(url: str, is_container: bool) -> str:
    ...
    """
    # 逻辑说明：仅在宿主机运行时把容器内 8080 地址替换为暴露端口；容器内保持服务发现地址。
...
def _csv(values: Any) -> str:
    ...
    # 逻辑说明：将列表或标量规范为逗号分隔文本，供 Hermes 环境变量消费；空值返回空字符串。
    if not values:
        return ""
...
def _resolve_active_model(cfg: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    """
    # 逻辑说明：根据默认 agent 的 provider/model 引用定位活动模型；找不到时返回 None。
...
def _resolve_context_window(cfg: Dict[str, Any]) -> Optional[int]:
    # 逻辑说明：从活动模型读取正整数上下文窗口；缺失或无效时返回 None 交由 Hermes 默认处理。
...
def _resolve_vision_enabled(cfg: Dict[str, Any]) -> bool:
    ...
    # 逻辑说明：综合活动模型声明和输入模态判断视觉能力，配置缺失时默认为关闭。
    m = _resolve_active_model(cfg)
    if m is None:
        return False
...
def _is_bridge_owned(key: str) -> bool:
    # 逻辑说明：判断环境变量是否由桥接器管理，重写 env 时只替换这些键而保留用户变量。
...
def _quote_env_value(value: str) -> str:
    ...
    # 逻辑说明：为 dotenv 值选择安全引用和转义方式，使空格、引号、换行读取后不变形。
    if value is None:
        return '""'
    s = str(value)
    s = s.replace("\\", "\\\\").replace('"', '\\"')
...
def _read_env_file(path: Path) -> Dict[str, str]:
    ...
    dropped silently.
    """
    # 逻辑说明：逐行解析 dotenv，同时保留注释和空行；文件缺失返回空列表。
    if not path.exists():
        return {}
    out: Dict[str, str] = {}
...
def _write_env_file(path: Path, env: Dict[str, str]) -> None:
    ...
    """
    # 逻辑说明：保留非桥接变量与注释，再追加排序后的受管变量，通过临时文件原子替换。
    lines = [
...
def _matrix_env(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    """
    # 逻辑说明：提取 Matrix 连接、身份、房间和 ACL，转换为 Hermes 环境变量。
...
def _model_env(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    """
    # 逻辑说明：将活动模型 provider、名称、网关和上下文窗口转换为 Hermes LLM 环境变量。
    m = _resolve_active_model(cfg)
    if not m:
        return {}
    in_container = _is_in_container()
...
def _runtime_env(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    """
    # 逻辑说明：只复制 AgentTeams/Hermes 相关环境键，避免任意 OpenClaw 环境泄漏给进程。
...
def _model_yaml_block(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 逻辑说明：把活动模型渲染成 Hermes config.yaml 的 model 区块；无模型时返回空字典。
    m = _resolve_active_model(cfg)
    if not m:
        return {}
...
def _matrix_yaml_block(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 逻辑说明：把 Matrix homeserver、用户、房间等信息渲染为 Hermes gateway YAML 区块。
    matrix_raw = cfg.get("channels", {}).get("matrix", {})
    if not matrix_raw:
        return {}
    block: Dict[str, Any] = {
...
def _auxiliary_vision_yaml_block(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    # 逻辑说明：仅在主模型支持视觉且配置辅助模型时生成视觉 YAML，否则不启用额外链路。
...
def _logging_yaml_block() -> Dict[str, Any]:
    ...
    """
    # 逻辑说明：仅在调试开关开启时生成详细日志 YAML，默认不改变 Hermes 日志级别。
...
def bridge_openclaw_to_hermes(
    ...
    # 逻辑说明：读取 OpenClaw 配置，生成 Hermes dotenv/YAML 和目录；受管文件可重复安全更新。
