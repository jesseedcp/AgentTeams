...
File Sync Design Principle (mirrors copaw-worker):
  ...
  Controller-managed (Worker read-only, pull only):
...
class FileSync:
    ...
    def pull_all(self) -> list[str]:
        """Pull Controller-managed files only. Returns changed filenames.
...
def push_local(sync: FileSync, since: float = 0) -> list[str]:
    ...
    Excludes Controller-managed files only. AGENTS.md, SOUL.md, .hermes/sessions/
    ...
    don't fight the Controller on every restart:
      - .hermes/config.yaml
      - .hermes/.env
    """
    _EXCLUDE_FILES = {
    ...
    _EXCLUDE_DIRS = {
        ...
        "logs",
        "__pycache__",
        # Controller-managed shared directory (pulled from bucket root)
        "shared",
    }
    _EXCLUDE_EXTENSIONS = {".lock", ".db-journal", ".db-wal", ".db-shm"}
    # Derived files inside .hermes/ that are generated by bridge.py — must
    # not be pushed back, otherwise Controller-driven openclaw.json updates
