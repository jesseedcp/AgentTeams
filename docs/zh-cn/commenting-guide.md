...
本项目把“函数级覆盖”进一步定义为：手写函数只要包含多步执行、分支或循环、多个下游调用、
异步等待、I/O、持久状态、权限判断或外部副作用，就必须在函数入口附近有一段具体的中文
`逻辑说明`。说明至少要让读者知道该函数正在处理什么真实对象、返回或推进什么状态，以及
失败如何暴露或恢复。纯 getter、常量返回、Protocol/overload 声明、明确的 no-op 和生成代码
不强制添加。相同的泛化句子不能复制到多个不同行为的函数；“读取参数和内部状态后返回结果”
这类文字即使能被计数器发现，也不算合格覆盖。
