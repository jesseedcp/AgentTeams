...
日期：2026-07-28  
状态：已批准执行  
基线：`agentscope-ai/AgentTeams@fb3a40be1f005bd584f45544fc73bd4601d5c52a`（2026-07-31 远程 `main`）
...
- `/elevated` 只允许在管理员私聊调整确认策略；`full` 可取消该房间的全部
  工具级审批，但永远不能让普通成员获得管理员权限，也不能扩大工具集合、
  资源范围或绕过路径与凭据保护。
